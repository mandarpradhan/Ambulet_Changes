...
/*This function reads the array data which has dict idbus_message_iter_recurse(&subiter, &dict_iter);
                if (dbus_message_iter_get_arg_type(&dict_iter) ==
                    DBUS_TYPE_STRING) {
in it, dict in turn has object path */
...
int read_array_dict(DBusMessageIter * args, char *list[30])
{
		...
		dbus_message_iter_recurse(args, &subiter);
				...
				num_of_dev = read_obj(&dict_iter, list);	
}
return(num_of_dev);
}
...
void read_dict_variant(DBusMessageIter * args, int *rssi)
{
        DBusMessageIter dict_iter, var_iter, subiter, int_iter;
        char str_ptr[50];
	int value;
        int type = dbus_message_iter_get_arg_type(args);
        ...
        do {
                ...
                if (dbus_message_iter_get_arg_type(&dict_iter) ==
                    DBUS_TYPE_STRING) {
                        ...
                        if (!strcmp(str_ptr, "signal-quality")) {
                                dbus_message_iter_next(&dict_iter);
                                int dict_type = dbus_message_iter_get_arg_type(&dict_iter);
                                if (dict_type == DBUS_TYPE_VARIANT) {
				       dbus_message_iter_recurse(&dict_iter, &var_iter);

                                       dict_type = dbus_message_iter_get_arg_type(&var_iter);
                                       if (dict_type == DBUS_TYPE_STRUCT)
                                       		dbus_message_iter_recurse(&var_iter, &int_iter);
                				read_int_byte(&int_iter, rssi);
                                }
                        }
                }
        } while (dbus_message_iter_next(&subiter));
...
//This function reads the service provider name
