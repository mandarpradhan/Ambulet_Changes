...
#ifndef IW_RESTRIC_ENUM
				fprintf(stderr,
					"Cannot parse " PROC_NET_DEV "\n");
#else
				fprintf(stderr,
					"Cannot parse " PROC_NET_WIRELESS "\n");
#endif
			} else
 ...
 */
static int print_iface_version_info(int skfd, char *ifname, char *args[],	/* Command line args */
				    int count)
{				/* Args count */
	struct iwreq wrq;
	...
	if (iw_get_ext(skfd, ifname, SIOCGIWRANGE, &wrq) < 0) {
		...
		fprintf(stderr,
			"%-8.16s  Driver has no Wireless Extension version information.\n\n",
			ifname);
		return (0);
	}
...
int iw_set_basic_config(int skfd, const char *ifname, wireless_config * info)
{
	...
	if (info->has_key) {
		...
		if ((flags & IW_ENCODE_INDEX) > 0) {
			...
			wrq.u.data.flags =
			    (flags & (IW_ENCODE_INDEX)) | IW_ENCODE_NOKEY;
			wrq.u.data.length = 0;
...
iw_print_freq(char *buffer,
	      int buflen, double freq, int channel, int freq_flags)
{
...
iw_get_stats(int skfd,
	     const char *ifname,
	     iwstats * stats, const iwrange * range, int has_range)
{
...
iw_print_stats(char *buffer,
	       int buflen,
	       const iwqual * qual,
	       const iwrange * range, int has_range, int *level)
{
	int len;
			...
			if (!(qual->updated & IW_QUAL_LEVEL_INVALID)) {
				...
				len =
				    snprintf(buffer, buflen,
					     "Signal level%c%g dBm  ",
					     qual->
					     updated & IW_QUAL_LEVEL_UPDATED ?
					     '=' : ':', rcpilevel);
				*level = rcpilevel;
				printf(" %s\n", buffer);
				buffer += len;
			...
			if (!(qual->updated & IW_QUAL_NOISE_INVALID)) {
				...
				len =
				    snprintf(buffer, buflen,
					     "Noise level%c%g dBm",
					     qual->
					     updated & IW_QUAL_NOISE_UPDATED ?
					     '=' : ':', rcpinoise);
				printf("noise in print_stats %d\n",
				       (int)rcpinoise);
				printf(" %s\n", buffer);
			}
		} else {
...
iw_in_key_full(int skfd,
	       const char *ifname,
	       const char *input, unsigned char *key, __u16 * flags)
{
	int keylen = 0;
	char *p;
		...
		*p = '\0';
		...
		if (range.we_version_compiled > 15) {
			...
			if ((*flags & IW_ENCODE_INDEX) == 0) {
				...
				*flags |=
				    range.
				    encoding_login_index & IW_ENCODE_INDEX;
			}
			printf("flags = %X, index = %X\n", *flags,
			       range.encoding_login_index);
		}
...
iw_print_pm_value(char *buffer,
		  int buflen, int value, int flags, int we_version)
{
	/* Check size */
	if (buflen < 25) {
		snprintf(buffer, buflen, "<too big>");
...
iw_print_timeval(char *buffer,
		 int buflen,
		 const struct timeval *timev, const struct timezone *tz)
{
	int s;
...
char *iw_sawap_ntop(const struct sockaddr *sap, char *buf)
{
	...
	const struct ether_addr *ether_wap =
	    (const struct ether_addr *)sap->sa_data;
...
int iw_in_inet(char *name, struct sockaddr *sap)
{
	...
	memcpy((char *)&sain->sin_addr, (char *)hp->h_addr_list[0],
	       hp->h_length);
...
int iw_in_addr(int skfd, const char *ifname, char *bufp, struct sockaddr *sap)
{
	...
	if (strchr(bufp, ':') == NULL) {
		...
		if (iw_check_if_addr_type(skfd, ifname) < 0) {
			fprintf(stderr,
				"%-8.16s  Interface doesn't support IP addresses\n",
				ifname);
			return (-1);
		}
		...
		memcpy((char *)&(arp_query.arp_pa),
		       (char *)&if_address, sizeof(struct sockaddr));
		...
		memcpy((char *)sap,
		       (char *)&(arp_query.arp_ha), sizeof(struct sockaddr));
			    ...
			    int we_version)
{
...
#ifdef DEBUG
	printf
	    ("DBG - stream->current = %p, stream->value = %p, stream->end = %p\n",
	...
	if (event_type == IW_HEADER_TYPE_POINT) {
		...
		unsigned int extra_len =
		    iwe->len - (event_len + IW_EV_LCP_PK_LEN);
		if (extra_len > 0) {
			...
			if (descr == NULL)
				...
				unsigned int token_len =
				    iwe->u.data.length * descr->token_size;
				...
				if ((token_len != extra_len)
					...
					alt_token_len =
					    alt_dlen.value * descr->token_size;
#ifdef DEBUG
...
#ifdef DEBUG
				printf
				    ("DBG - extra_len = %d, token_len = %d, token = %d, max = %d, min = %d\n",
				     extra_len, token_len, iwe->u.data.length,
				     descr->max_tokens, descr->min_tokens);
#endif
			}
		} else
			/* No data */
 ...
 */
static inline struct wireless_scan *iw_process_scanning_token(struct iw_event
							      *event,
							      struct
							      wireless_scan
							      *wscan)
{
	...
	case SIOCGIWAP:
		...
		wscan =
		    ...
		    malloc(sizeof(struct wireless_scan));
		if (wscan == NULL)
			return (wscan);
		...
		wscan->has_ap_addr = 1;
		memcpy(&(wscan->ap_addr), &(event->u.ap_addr),
		       sizeof(sockaddr));
		break;
	case SIOCGIWNWID:
		wscan->b.has_nwid = 1;
	...
	case IWEVQUAL:
		...
		wscan->has_stats = 1;
		memcpy(&wscan->stats.qual, &event->u.qual,
		       sizeof(struct iw_quality));
		break;
	case SIOCGIWRATE:
		...
		if ((!wscan->has_maxbitrate) ||
			...
			memcpy(&(wscan->maxbitrate), &(event->u.bitrate),
			       sizeof(iwparam));
		}
	case IWEVCUSTOM:
...
iw_process_scan(int skfd,
		char *ifname, int we_version, wireless_scan_head * context)
{
	struct iwreq wrq;
