...
pthread_t local_vlc, vlc_server, rtp_thread, rtcp_ltor_thread, rtcp_rtol_thread,
    rtsp_reconnect;
static unsigned short rtp_port_no, rtcp_client_port, rtcp_server_port;
...
void *rtsp_vlc()
{
	...
	int optval =
	    1, len, i, ret, vlc_listen, recv_len, vlc_send, my_network_view;
	char vlc_buff[1500];
#if 0
	while (1) {
	...
	while (1) {
		...
		vlc_connsock =
		    accept(vlc_sockfd, (struct sockaddr *)NULL, (int *)NULL);
		if (vlc_connsock < 0) {
 ...
 *
 */
void *localvlc(int *arg)
{
	int my_vlc_network_view = 0, right_send_len = 0;
...
void rtp_packet_handle (unsigned char buf[], int len)
{
if (!rtp_started) {
          update_rtp_started(buf);
         }
if (rtp_started == 1) {
         save_rtp_header(buf, len);
        }
}
...
void *vlcserver(int *Sock)
{
	...
	while (1) {
		...
		if (my_vlc_network_view != real_network_view) {
			...
			// seamless_conn();
			if (pthread_create
			    (&rtsp_reconnect, NULL, reconnect, NULL)) {
				printf
...
void get_rtsp_port_no(char req[])
{
	...
	sscanf(portnums, "client_port=%d-%d", &p1, &p2);
	rtcp_client_port = p2;
	printf("CLIENT PORT = %d\n", rtcp_client_port);

	portnums = strstr(req, "server_port=");
	sscanf(portnums, "server_port=%d-%d", &p1, &p2);
	printf("p1=%d p2=%d\n", p1, p2);
	rtcp_server_port = p2;
...
void reconnect()
{
	...
	char buf[PKT_BUFSIZ];
	int iresult, right_send_len, right_recv_len, goodbye_sockfd, optval = 1;
	struct sockaddr_in remote_addr, right_addr;
	static unsigned char goodbye_pkt[8] = {
		0x81, 0xcb, 0x00, 0x01, 0, 0,
		0, 0
	};
...
void save_rtp_header(unsigned char rtp_packet[], int pkt_len)
{
	...
	unsigned char *tmp;
	unsigned short right_seq_no, pkt_size;
	int right_timestamp, curr_len = 0, i = 0;

//              printf("%x %x %x %x %x %x %x %x %x %x %x %x\n",
//              rtp_packet[0],rtp_packet[1],rtp_packet[2],rtp_packet[3],rtp_packet[4],rtp_packet[5],rtp_packet[6],rtp_packet[7],rtp_packet[8],rtp_packet[9]
//              ,rtp_packet[10], rtp_packet[11]);

	tmp = rtp_packet;

	if (rtp_diff < 0) {
		if (abs(rtp_diff) >= pkt_len) {
			printf
			    ("^^^^^^^^^^^^^^^^^^^^^^^rtp_diff = pkt_len returning\n");
			rtp_diff = 0;
			return;
		}
		tmp = tmp + abs(rtp_diff);
		if (tmp[4] == 0x80 && ((tmp[5] == 0xe0) || (tmp[5] == 0x60))) {
			curr_len = curr_len + abs(rtp_diff);
//         printf ("%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n",tmp[-5],tmp[-4],tmp[-3],tmp[-2],tmp[-1],tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5],tmp[6],tmp[7],tmp[8],tmp[9],tmp[10],tmp[11]); 
		} else {
			printf("Stradeled packet not a rtp packet!\n");
			rtp_diff = 0;
			return;
		}
	}
...
//      printf("currlen %d rtp_diff %d pkt_len : %d\n", curr_len, rtp_diff, pkt_len);
...
//      printf("------------------------------%d-----------------------------------------------------------\n", i++);
		...
		memcpy(&pkt_size, tmp + 2, 2);

		curr_len += ntohs(pkt_size) + 4;
		rtp_diff = pkt_len - curr_len;

		if (ntohs(pkt_size) == 0) {
			printf("%%%%%%%%%%%%%%%%packet size = 0 returning\n");
			return;
		}
		if (ntohs(pkt_size) > pkt_len) {
			printf("*******pkt_size %d  pkt_len %d curr_len %d\n",
			       ntohs(pkt_size), pkt_len, curr_len);
			return;
		}
		...
		if (tmp[4] == 0x80 && ((tmp[5] == 0xe0) || (tmp[5] == 0x60))) {
			...
			recv_rtp_wall_clock =
			    ...
			    (wall_clock_time.tv_usec / 1000000.0);
...
*/
			...
			if (rtp_ssrc) {
				printf
				    ("Incoming SSRC : %d %02X %02X %02X %02X ",
				     ntohl(tmp[12]), tmp[12], tmp[13], tmp[14],
				     tmp[15]);
				printf("Stored SSRC : %d %d\n", rtp_ssrc,
				       ntohl(rtp_ssrc));
			}
